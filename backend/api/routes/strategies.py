...
from backend.services.strategies.strategy_manager import (
    StrategyManager, StrategyRequest, execute_user_strategy
)
...
class StrategyExecutionRequest(BaseModel):
    ...
    strategy_type: str = Field(..., description="Strategy type (atr_options, dca_conservative, etc.)")
    ...
    profit_target: float = Field(..., gt=0, le=1, description="Profit target as percentage (0.20 = 20%)")
    stop_loss: Optional[float] = Field(0.05, gt=0, le=1, description="Stop loss as percentage (0.05 = 5%)")
    reinvest_percentage: Optional[float] = Field(0.80, ge=0, le=1, description="Reinvestment percentage (0.80 = 80%)")
    max_position_size: Optional[float] = Field(0.10, gt=0, le=1, description="Max position size as percentage (0.10 = 10%)")
    symbols: Optional[List[str]] = Field(None, description="Specific symbols to trade")
    custom_params: Optional[Dict[str, Any]] = Field(None, description="Strategy-specific parameters")
...
async def execute_strategy(
    ...
    """
    try:
        logger.info(f"🚀 Strategy execution request: {request.strategy_type} for user {user.id}")
...
async def get_atr_options_config(
    ...
    try:
        ...
        suggested_config = {
            "symbol": symbol,
            "atr_data": atr_data,
            "suggested_profit_target": 0.15 if atr_data['volatility_level'] == 'LOW' else 0.25,
            "suggested_stop_loss": 0.03 if atr_data['volatility_level'] == 'LOW' else 0.08,
            ...
            "risk_assessment": {
                ...
                "recommended_capital": 5000 if atr_data['volatility_level'] == 'LOW' else 10000
            }
        }
...
async def get_dca_config(
    strategy_variant: str = "conservative",
    user: User = Depends(get_current_user)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if strategy_variant not in dca_configs:
            raise HTTPException(status_code=400, detail=f"Unknown DCA variant: {strategy_variant}")
...
async def get_strategies_performance(
    ...
    """
    try:
        from backend.models.strategy_integration import StrategyExecution, StrategyStatus
        ...
        successful_executions = [e for e in executions if e.execution_summary and e.execution_summary.get('status') == 'SUCCESS']
        ...
        performance_summary = {
            ...
            "total_capital_deployed": sum(float(e.starting_capital) for e in executions),
        ...
        for execution in executions:
            ...
            by_strategy[strategy_type]["total_capital"] += float(execution.starting_capital)
