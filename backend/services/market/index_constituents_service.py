...
class IndexConstituentsService:
    ...
    async def get_index_constituents(self, index_name: str) -> List[str]:
            ...
            try:
                ...
                if cached_data:
                    ...
                    if datetime.now() - cached_time < timedelta(seconds=self.cache_ttl):
                        logger.info(f"✅ Using cached {index_name} constituents: {len(symbols)} symbols")
                        return symbols
        ...
        try:
            ...
            if symbols:
                ...
                logger.info(f"✅ Got {len(clean_symbols)} {index_name} constituents from {data_source}")
    ...
    async def get_all_tradeable_symbols(self, indices: List[str] = None) -> Dict[str, List[str]]:
        """
        ...
        """
        if indices is None:
            indices = ['SP500', 'NASDAQ100', 'DOW30']  # Skip Russell 2000 by default (too large)
    ...
    async def get_universe_for_atr(self) -> List[str]:
        ...
        logger.info(f"🚀 ATR universe ready: {len(universe_list)} symbols from major indices")
    ...
    async def _get_russell2000_constituents(self) -> tuple[List[str], str]:
        ...
        logger.info("Consider focusing on S&P 500 + NASDAQ 100 for now (~600 quality stocks)")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        logger.warning(f"FMP API error {response.status} for {endpoint}")
        except Exception as e:
            ...
            for table in tables:
                ...
                if symbol_col:
                    ...
                    if len(clean_symbols) > 50:  # Should have ~100 symbols
                        logger.info(f"✅ Wikipedia NASDAQ 100: {len(clean_symbols)} symbols")
                        return clean_symbols
        except Exception as e:
            ...
            for table in tables:
                if 'Symbol' in table.columns and len(table) <= 35:
                    ...
                    if 25 <= len(clean_symbols) <= 35:
                        logger.info(f"✅ Wikipedia Dow 30: {len(clean_symbols)} symbols")
                        return clean_symbols
        except Exception as e:
    ...
    def _validate_symbols(self, symbols: List[str]) -> List[str]:
        ...
        for symbol in symbols:
            if isinstance(symbol, str):
                ...
                # Basic validation
                if (len(symbol) <= 5 and 
                    symbol.isalnum() or '-' in symbol or '.' in symbol):
