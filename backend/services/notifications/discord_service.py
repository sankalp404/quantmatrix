...
class DiscordNotifier:
    ...
    async def send_entry_signal(
        ...
        if atr_value:
            market_data += f"📈 **ATR:** ${atr_value:.2f} ({(atr_value/price*100):.1f}%)\n"
        if rsi:
        ...
        embed.set_footer(text="QuantMatrix • ATR Matrix Strategy • Not Financial Advice")
        embed.set_timestamp()
    ...
    async def send_morning_brew(
        ...
        if top_opportunities:
            opportunities_text = "🎯 **HIGH-PROBABILITY SETUPS WITH QUANTIFIED TARGETS**\n\n"
            ...
            for i, opp in enumerate(top_opportunities[:4], 1):
                ...
                confidence_emoji = "🔥" if confidence >= 0.8 else "💪" if confidence >= 0.65 else "👍"
                ...
                opportunities_text += f"🛑 **Stop:** ${stop_loss:.2f} (-{risk_amount:.1f}%)\n"
        ...
        if economic_calendar:
            calendar_text = f"📅 **KEY EVENTS TODAY ({today.strftime('%B %d, %Y')})**\n\n"
            ...
            for event in economic_calendar:
                ...
                if forecast != 'N/A':
                    calendar_text += f"   📊 Forecast: {forecast} | Previous: {previous}\n"
                calendar_text += "\n"
        ...
        sentiment_text += f"🧠 **Market Psychology:** {market_sentiment['sentiment_label']}"
        ...
        # Trading Outlook
        embed.add_embed_field(
            name="🔮 QuantMatrix Trading Outlook",
    ...
    async def send_enhanced_portfolio_digest(
        ...
        for position in positions:
            if position.get('unrealized_pnl_pct', 0) > 15:
                action_items.append(f"🎯 **{position['symbol']}**: Consider scaling out (+{position['unrealized_pnl_pct']:.1f}%)")
            elif position.get('unrealized_pnl_pct', 0) < -8:
                action_items.append(f"⚠️ **{position['symbol']}**: Review stop loss ({position['unrealized_pnl_pct']:+.1f}%)")
        ...
        if holdings_news:
            ...
            for news_item in holdings_news[:3]:
                news_text += f"📰 **{news_item['symbol']}**: {news_item['headline'][:60]}...\n"
    ...
    async def send_post_market_brew(
        ...
        if morning_predictions and len(morning_predictions) > 1:
            ...
            for pred in morning_predictions[:3]:
                if pred.get('symbol') != 'Example tracking coming soon':
                    result_emoji = "✅" if (pred['predicted_direction'] == 'UP' and pred['actual_change'] > 0) or (pred['predicted_direction'] == 'DOWN' and pred['actual_change'] < 0) else "❌"
        ...
        # Market sentiment
        sentiment_emoji = "😊" if market_sentiment.get('sentiment_label') == 'Bullish' else "😟" if market_sentiment.get('sentiment_label') == 'Bearish' else "😐"
        embed.add_embed_field(
            ...
            if alerts:
                ...
                if top_alerts:
                    ...
                    for alert in top_alerts:
                        account_text += f"• {alert['symbol']}: {alert['message'][:50]}...\n"
    ...
    async def send_portfolio_digest(
        ...
        if sorted_positions:
            ...
            for pos in top_performers:
                if pos.get('unrealized_pnl_pct', 0) > 0:
                    performers_text += f"📈 **{pos['symbol']}**: {pos['unrealized_pnl_pct']:+.1f}%\n"
            ...
            for pos in bottom_performers:
                if pos.get('unrealized_pnl_pct', 0) < 0:
                    underperformers_text += f"📉 **{pos['symbol']}**: {pos['unrealized_pnl_pct']:+.1f}%\n"
        ...
        if holdings_news:
            ...
            for news in holdings_news[:3]:
                sentiment_emoji = {"positive": "📈", "negative": "📉", "neutral": "📰"}.get(news.get('sentiment', 'neutral'), "📰")
