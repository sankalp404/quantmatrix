...
from backend.config import settings, INDICATORS_CONFIG
...
class MarketDataService:
    def __init__(self):
        ...
        self.cache_ttl = getattr(settings, 'MARKET_DATA_CACHE_TTL', 300)  # Default 5 minutes
        ...
        self.rate_limits = {
            ...
            APIProvider.TWELVE_DATA: {"calls": [], "max_per_day": 800, "daily_count": 0, "last_reset": datetime.now().date()},
            APIProvider.FMP: {"calls": [], "max_per_day": 250, "daily_count": 0, "last_reset": datetime.now().date()},
            APIProvider.YFINANCE: {"calls": [], "max_per_minute": 2000}  # Very generous limit
        }
    ...
    def _check_rate_limit(self, provider: APIProvider) -> bool:
        ...
        if "max_per_minute" in limits:
            limits["calls"] = [call_time for call_time in limits["calls"] if now - call_time < 60]
    ...
    async def _get_best_provider_for_data_type(self, data_type: str) -> APIProvider:
        ...
        if data_type == "real_time_quote":
            for provider in [APIProvider.FINNHUB, APIProvider.FMP, APIProvider.YFINANCE]:
                if self._check_rate_limit(provider) and self._is_provider_available(provider):
                    return provider
                    
        elif data_type == "technical_indicators":
            for provider in [APIProvider.TWELVE_DATA, APIProvider.FMP, APIProvider.YFINANCE]:
                if self._check_rate_limit(provider) and self._is_provider_available(provider):
                    return provider
                    
        elif data_type == "historical_data":
            for provider in [APIProvider.YFINANCE, APIProvider.TWELVE_DATA, APIProvider.FMP, APIProvider.FINNHUB]:
                if self._check_rate_limit(provider) and self._is_provider_available(provider):
                    return provider
                    
        elif data_type == "company_info":
            for provider in [APIProvider.FMP, APIProvider.YFINANCE, APIProvider.FINNHUB]:
                if self._check_rate_limit(provider) and self._is_provider_available(provider):
                    return provider
    ...
    async def get_historical_data(
        ...
        try:
            ...
            if data is not None and not data.empty:
                ...
                self.redis_client.setex(cache_key, cache_seconds, data.to_json(orient='index'))
                return data
    ...
    async def _get_historical_yfinance(self, symbol: str, period: str, interval: str) -> Optional[pd.DataFrame]:
    ...
    async def _get_historical_twelve_data(self, symbol: str, period: str, interval: str) -> Optional[pd.DataFrame]:
    ...
    async def _get_historical_fmp(self, symbol: str, period: str, interval: str) -> Optional[pd.DataFrame]:
    ...
    async def get_technical_analysis(self, symbol: str) -> Dict:
        ...
        try:
            ...
            logger.debug(f"Processing {symbol}: Current price=${current_close:.2f}, Data points={len(data)}")
            ...
            if not tech_indicators or len(tech_indicators) < 3:
                logger.info(f"API indicators failed for {symbol}, using local calculations")
            ...
            if missing_indicators:
                logger.warning(f"Missing critical indicators for {symbol}: {missing_indicators}")
            ...
            logger.debug(f"Successfully calculated indicators for {symbol}: {list(indicators.keys())}")
            return indicators
            
        except Exception as e:
            logger.error(f"Error in technical analysis for {symbol}: {e}", exc_info=True)
    ...
    async def _get_technical_indicators(self, symbol: str, data: pd.DataFrame) -> Dict:
        ...
        try:
            provider = await self._get_best_provider_for_data_type("technical_indicators")
    ...
    async def _get_indicators_twelve_data(self, symbol: str) -> Dict:
        ...
        try:
            ...
            # RSI
            rsi = self.twelve_data_client.rsi(symbol=symbol, interval="1day", time_period=14)
            if rsi.status == "ok":
            ...
            # ATR
            atr = self.twelve_data_client.atr(symbol=symbol, interval="1day", time_period=14)
            if atr.status == "ok":
            ...
            for period in [10, 20, 50, 100, 200]:
                ...
                ma = self.twelve_data_client.ema(symbol=symbol, interval="1day", time_period=period) if ma_type == "ema" else \
                     self.twelve_data_client.sma(symbol=symbol, interval="1day", time_period=period)
            ...
            # ADX
            adx = self.twelve_data_client.adx(symbol=symbol, interval="1day", time_period=14)
            if adx.status == "ok":
    ...
    async def _get_indicators_fmp(self, symbol: str) -> Dict:
        ...
        try:
            ...
            # RSI
            rsi_data = fmpsdk.rsi(apikey=settings.FMP_API_KEY, symbol=symbol, period=14, datatype="json")
            if rsi_data and len(rsi_data) > 0:
                indicators['rsi'] = rsi_data[0].get('rsi')
            
            # SMA
            for period in [20, 50, 100, 200]:
                sma_data = fmpsdk.sma(apikey=settings.FMP_API_KEY, symbol=symbol, period=period, datatype="json")
                if sma_data and len(sma_data) > 0:
                    indicators[f'sma_{period}'] = sma_data[0].get('sma')
            
            # EMA
            ema_data = fmpsdk.ema(apikey=settings.FMP_API_KEY, symbol=symbol, period=10, datatype="json")
    ...
    def _manual_indicator_calculations(self, data: pd.DataFrame) -> Dict:
        ...
        try:
            logger.info(f"Using manual calculations for indicators, data length: {len(data)}")
            ...
            for period in [20, 50, 100, 200]:
                if len(data) >= period:
                    ...
                    if not sma.empty and not pd.isna(sma.iloc[-1]):
                        ...
                        logger.debug(f"Calculated SMA_{period}: {indicators[f'sma_{period}']:.2f}")
        ...
        logger.info(f"Manual calculations completed. Indicators: {list(indicators.keys())}")
        return indicators
    
    def _calculate_atr_matrix_metrics(self, data: pd.DataFrame, indicators: Dict) -> Dict:
        """Calculate ATR Matrix specific metrics."""
        metrics = {}
        ...
        try:
            ...
            logger.debug(f"ATR Matrix calculation - Price: {current_price}, SMA50: {sma_50}, ATR: {atr}")
            ...
            if current_price and sma_50 and atr and atr > 0:
                ...
                logger.debug(f"ATR distance: {atr_distance:.2f}, ATR percent: {atr_percent:.2f}%")
            else:
                logger.warning(f"Missing critical data for ATR Matrix: price={current_price}, sma50={sma_50}, atr={atr}")
            ...
            if all(ma is not None for ma in mas):
                ...
                logger.debug(f"MA alignment: {ma_aligned} ({ma_names} = {[f'{ma:.2f}' if ma else 'None' for ma in mas]})")
            else:
                logger.warning(f"Missing MAs for alignment check: {dict(zip(ma_names, mas))}")
            ...
            if len(data) >= 20:
                ...
                if high_20 > low_20:
                    ...
                    logger.debug(f"20D price position: {price_position:.1f}% (H:{high_20:.2f}, L:{low_20:.2f})")
        ...
        for provider in APIProvider:
            ...
            try:
                if provider == APIProvider.YFINANCE:
                    ...
                    quote = fmpsdk.quote(apikey=settings.FMP_API_KEY, symbol=test_symbol)
                    if quote and len(quote) > 0:
                        logger.info(f"Health check passed for {provider.value}")
                        return True
...
POPULAR_STOCKS = [
    ...
    # Healthcare
    "JNJ", "PFE", "UNH", "MRK", "ABBV", "TMO", "MDT", "BMY", "AMGN", "GILD",
    # Consumer
    "KO", "PEP", "WMT", "TGT", "HD", "LOW", "MCD", "SBUX", "NKE", "DIS",
    # Industrial
    "BA", "CAT", "GE", "MMM", "UPS", "FDX", "HON", "LMT", "RTX", "DE",
    # Energy
    "XOM", "CVX", "COP", "EOG", "SLB", "PSX", "VLO", "MPC", "OXY", "HAL",
    # Communications
    "VZ", "T", "CMCSA", "CHTR", "TMUS", "DISH",
    # Real Estate
    "AMT", "PLD", "CCI", "EQIX", "SPG", "O", "WELL", "AVB", "EQR", "BXP",
    # Utilities
    "NEE", "DUK", "SO", "D", "AEP", "EXC", "XEL", "ED", "ETR", "ES",
    # Materials
    "LIN", "APD", "SHW", "ECL", "FCX", "NEM", "DOW", "DD", "PPG", "IFF",
    # Technology
    "ADBE", "INTC", "CSCO", "IBM", "QCOM", "TXN", "AVGO", "MU", "AMD", "NOW"
]
...
class FreeMarketDataService:
    ...
    async def get_real_time_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            logger.warning(f"❌ Could not get real-time price for {symbol} from free sources")
            return None
    ...
    async def _get_yahoo_price(self, symbol: str) -> Optional[float]:
        ...
        try:
            url = f"{self.yahoo_base_url}/v8/finance/chart/{symbol}?range=1d&interval=1m"
    ...
    async def get_options_chain_from_tastytrade(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if options_data:
                logger.info(f"✅ TastyTrade options data for {symbol}: {len(options_data)} contracts")
                return options_data
    ...
    async def update_portfolio_with_real_time_prices(self, holdings: List[Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for holding in holdings:
                ...
                if holding.symbol in real_time_prices:
                    ...
                    real_time_pnl_pct = (real_time_pnl / cost_basis * 100) if cost_basis > 0 else 0
            ...
            logger.info(f"✅ Updated {len(updated_holdings)} holdings with real-time data from FREE sources")
