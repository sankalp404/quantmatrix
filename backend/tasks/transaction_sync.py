...
def sync_account_transactions(self, account_number: str, days: int = 365):
    ...
    try:
        logger.info(f"Starting transaction sync for account {account_number} ({days} days)")
        
        import asyncio
        result = asyncio.run(transaction_sync_service.sync_account_transactions(account_number, days))
        
        logger.info(f"Transaction sync completed for {account_number}: {result}")
        return result
        
    except Exception as e:
        logger.error(f"Error in sync_account_transactions task for {account_number}: {e}")
