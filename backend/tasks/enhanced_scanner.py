    ...
    try:
        ...
        for stock in stock_universe:
            ...
            if cached_analysis:
                ...
                cache_misses += 1
                analysis_tasks.append(perform_comprehensive_analysis(symbol, db))
        ...
        entry_opportunities = [
            ...
            if analysis
            and analysis.get("entry_signal")
            and analysis.get("confidence_score", 0) >= 0.65
        ]
        ...
        logger.info(
            f"🎯 Enhanced scan completed: {signals_sent} signals in {scan_duration:.1f}s"
        )
        ...
        return {"status": "error", "error": str(e), "signals_sent": signals_sent}
    finally:
        db.close()
        ...
        if polygon_service.is_premium:
            async with polygon_service as poly:
                ...
                if comprehensive_data:
                    ...
                    analysis_result = {
                        "symbol": symbol,
                        "current_price": comprehensive_data.get("quote", {}).get(
                            "price", 0
                        ),
                        "market_cap": comprehensive_data.get("details", {}).get(
                            "market_cap"
                        ),
                        "sector": comprehensive_data.get("details", {}).get("sector"),
                        "industry": comprehensive_data.get("details", {}).get(
                            "industry"
                        ),
                        "company_synopsis": comprehensive_data.get("details", {}).get(
                            "description", ""
                        ),
                        ...
                        "stop_loss_price": comprehensive_data.get("quote", {}).get(
                            "price", 0
                        )
                        * 0.95,
...
async def enhanced_morning_brew(self):
    ...
    try:
        ...
        if (
            cached_brew
            and (datetime.utcnow() - cached_brew["cache_timestamp"]).total_seconds()
            < 3600
        ):
            ...
            return {
                ...
                "source": "cached",
                "cache_age_minutes": (
                    datetime.utcnow() - cached_brew["cache_timestamp"]
                ).total_seconds()
                / 60,
            }
