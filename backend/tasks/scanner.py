...
async def run_atr_matrix_scan(self):
    ...
    try:
        ...
        logger.info(f"ATR Matrix scan completed: {len(entry_opportunities)} opportunities found in {scan_time:.1f}s")
...
async def run_custom_scan(self, symbols: List[str], criteria: Dict):
    ...
    try:
        ...
        logger.info(f"Custom scan completed: {len(results)} results in {scan_time:.1f}s")
...
async def _analyze_symbol_for_atr_matrix(symbol: str) -> Dict:
    ...
    try:
        ...
        if entry_signals:
            ...
            result["entry_reasons"] = entry_signal.metadata.get('entry_reason', '').split('; ')
